...
export class FormEditUserComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Récupérer les données de l'utilisateur depuis les paramètres de l'URL
    this.route.params.subscribe(params => {
      const user: User = JSON.parse(params['user']); // Convertir la chaîne JSON en objet User
      this.editForm.patchValue(user); // Remplir le formulaire avec les données de l'utilisateur
    });
  }
  onSubmit() {
